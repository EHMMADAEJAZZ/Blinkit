...
const EditProduct = ({ data,fetchProduct }) => {
 ...
 const navigate= useNavigate()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await productApi.updateProduct(data?._id, formData);
  ...
  const handleAddMoreField = () => {
    const feeldExisted = Object?.keys(product?.moreDetails)?.find(
        ...
        <div className='w-full '>
          ...
          >
            ...
            <div className='col-span-full'>
              ...
              />
              {error.subcategory && (
                <p className='error'>{error.subcategory}</p>
              )}
            </div>
            <div
              className={`w-full  items-center gap-2 flex-wrap col-span-full border border-gray-300 bg-blue-50 p-2 transition-all duration-500 ${
            ...
            <div className='w-full flex  justify-between items-center my-2 '>
              <button
                type='submit'
                className={` bg-primary-200 text-white hover:bg-primary-100 px-2 lg:px-10 py-2 lg:py-3  rounded  uppercase text-xs sm:text-sm tracking-widest font-bold ${
                  isEditing
                    ? 'bg-red-600 text-white cursor-wait hover:bg-red-400'
                    : ''
                }`}
              >
